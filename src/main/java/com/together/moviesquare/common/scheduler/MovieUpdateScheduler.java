...
public class MovieUpdateScheduler {
	...
	public void movieInfoUpdate() throws InterruptedException, ExecutionException {
		...
		//1919
		for (int year = inputYear; year <= currentYear; year++) {
			log.info(year+"년 시작");
		...
		for(List<MovieResult> mList : listmList) {
			...
			for (MovieResult m : mList) {
				...
				//movieService.mergeIntoMovieListOne(m);
			}
		...
		*/
		...
		for (List<MovieResult> mList : listmList) {
		    ...
		    // 500개씩 나누기 위해 시작 인덱스
		    int batchSize = 500;
		    for (int i = 0; i < f; i += batchSize) {
		        // 500개씩 subList 만들기 (끝 인덱스가 리스트의 크기를 넘지 않도록 처리)
		        List<MovieResult> subList = mList.subList(i, Math.min(i + batchSize, f));

		        // 각 MovieResult 처리
		        for (MovieResult m : subList) {
		            m.setTitle(m.getTitle().trim());
		            m.setActorsNm(m.getActors().setActorsNm());
		            m.setStaffsNm(m.getStaffs().setStaffsNm());
		            m.setDistributor(m.getStaffs().setDistributor());
		            ...
		            // movieService.mergeIntoMovieListOne(m); // 이 부분 주석 유지
		        }

		        log.info("movie 업데이트 또는 삽입할 사이즈" + subList.size());

		        // 500개씩 병합 실행
		        movieService.mergeIntoMovieList(subList);
		    }
